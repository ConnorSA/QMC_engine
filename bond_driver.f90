!> @brief Driver for running multiple simulations to optimise bondlength
!!
...
program bond_length_driver
  ...
  ! bond loop parameters
  integer :: loop_bond
  integer :: n_bond_points
  real(dp) :: bond_lower, bond_upper
  real(dp), allocatable,dimension(:) :: bond_points
  real(dp), allocatable,dimension(:) :: bond_energies
  integer,dimension(1) :: best_bond_trial
  ! Start timers
  ...
  ! Start bond length loop
  bond_lower = 0.2_dp
  bond_upper = 5.0_dp
  n_bond_points = 50
  allocate(bond_points(n_bond_points))
  allocate(bond_energies(n_bond_points))
  
  do loop_bond = 1, n_bond_points
    bond_points(loop_bond) = (bond_upper -bond_lower)*loop_bond/n_bond_points + bond_lower
    print*,"bond length", bond_points(loop_bond)

    if (n_atoms_in == 1) then
    atom_coords_in(:,1)=[0.0_dp,0.0_dp,0.0_dp] ! 1 atom at origin
    else if (n_atoms_in == 2) then
    atom_coords_in(:,1)=[bond_points(loop_bond)/2.0_dp,0.0_dp,0.0_dp] !2 atoms evenly placed about origin
    atom_coords_in(:,2)=[-bond_points(loop_bond)/2.0_dp,0.0_dp,0.0_dp]
    end if
    ...
    bond_energies(loop_bond) = trial_energies(best_trial(1))
    
    deallocate(trials,trial_energies,x_0,mcmc_run)
    call deinitialise_basis
    
  end do 


  print*, "Loop through bond lengths complete"
  best_bond_trial = minloc(bond_energies)
  print*, "Bond length with minimum energy=",bond_points(best_bond_trial)
  print*, "Minimum energy=",bond_energies(best_bond_trial),"Hartree Energy"
  
   ! Write to text file

  OPEN (unit=101,file="energies.txt",action="write")
  do loop_bond = 1, n_bond_points
        write (101,*) bond_points(loop_bond),bond_energies(loop_bond)
  end do
  
    CLOSE (101)
    print*, "energies written to file"

  


  ! deallocation
  deallocate(atom_coords_in,proton_numbers_in)
 ! call deinitialise_basis
  deallocate(bond_points,bond_energies)
  ! Report Timings
...
end program bond_length_driver
